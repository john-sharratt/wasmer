...
use wasmer::{
    ...
    Store, WasmerEnv, TypedFunction, Reactors,
};
...
pub struct WasiEnv {
    ...
    #[wasmer(reactors)]
    reactors: LazyInit<Reactors>,
    /// Represents the callback for spawning a thread
    #[derivative(Debug = "ignore")]
    #[wasmer(export(optional = true, name = "_start_thread"))]
    thread_spawn: LazyInit<TypedFunction<u64, ()>>,
    /// Represents the callback for spawning a reactor
    #[derivative(Debug = "ignore")]
    #[wasmer(export(optional = true, name = "_react"))]
    react: LazyInit<TypedFunction<u64, ()>>,
    /// Represents the callback for destroying a local thread variable
    #[derivative(Debug = "ignore")]
    #[wasmer(export(optional = true, name = "_thread_local_destroy"))]
    thread_local_destroy: LazyInit<TypedFunction<(u64, u64), ()>>,
    /// Represents the callback for allocating memory
    #[derivative(Debug = "ignore")]
...
impl WasiEnv {
    ...
    fn new_ext(state: Arc<WasiState>) -> Self {
        let ret = Self {
            state: state,
            memory: LazyInit::new(),
            thread_spawn: LazyInit::new(),
            reactors: LazyInit::new(),
            react: LazyInit::new(),
            thread_local_destroy: LazyInit::new(),
            malloc: LazyInit::new(),
            free: LazyInit::new(),
...
fn generate_import_object_wasix32_v1(store: &Store, env: WasiEnv) -> Imports {
    ...
    imports! {
        "wasix_32v1" => {
            ...
            "thread_local_create" => Function::new_native_with_env(store, env.clone(), thread_local_create),
            "thread_local_destroy" => Function::new_native_with_env(store, env.clone(), thread_local_destroy),
            "thread_local_set" => Function::new_native_with_env(store, env.clone(), thread_local_set),
            "thread_local_get" => Function::new_native_with_env(store, env.clone(), thread_local_get),
