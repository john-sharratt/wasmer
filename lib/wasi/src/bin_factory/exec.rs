...
use crate::{WasiEnv, WasiFunctionEnv, runtime::SpawnedMemory, import_object_for_all_wasi_versions, WasiError};
...
for BinFactory
{
    fn spawn(&self, name: &str, store: Store, config: SpawnOptionsConfig<WasiEnv>, _fallback: &dyn VirtualBusSpawner<WasiEnv>) -> wasmer_vbus::Result<BusSpawnedProcess> {
        ...
        let (exit_code_tx, exit_code_rx) = mpsc::channel(1);
        {
            ...
            self.runtime.task_wasm(Box::new(move |mut store, module, memory| Box::pin(
                async move {
