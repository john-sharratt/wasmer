#![deny(dead_code)]
use crate::{WasiEnv, WasiError, WasiState};
...
pub(crate) fn thread_spawn(
    ...
    super::thread_spawn::<MemoryType>(env, user_data, reactor, ret_tid)
}

pub(crate) fn thread_local_create(
    env: &WasiEnv,
    user_data: u64,
    ret_key: WasmPtr<__wasi_tl_key_t, MemoryType>,
) -> __wasi_errno_t {
    super::thread_local_create(env, user_data, ret_key)
}

pub(crate) fn thread_local_destroy(
    env: &WasiEnv,
    key: __wasi_tl_key_t
) -> __wasi_errno_t {
    super::thread_local_destroy(env, key)
}

pub(crate) fn thread_local_set(
    env: &WasiEnv,
    key: __wasi_tl_key_t,
    val: __wasi_tl_val_t
) -> __wasi_errno_t {
    super::thread_local_set(env, key, val)
}

pub(crate) fn thread_local_get(
    env: &WasiEnv,
    key: __wasi_tl_key_t,
    ret_val: WasmPtr<__wasi_tl_val_t, MemoryType>,
) -> __wasi_errno_t {
    super::thread_local_get(env, key, ret_val)
}
...
pub(crate) fn futex_wake(
    ...
    super::futex_wake::<MemoryType>(env, futex, ret_woken)
}
...
pub(crate) fn futex_wake_all(
    ...
    super::futex_wake_all::<MemoryType>(env, futex, ret_woken)
}
...
pub(crate) fn bus_call(
    ...
    super::bus_call::<MemoryType>(
        env, bid, topic_hash, format, buf, buf_len, ret_cid,
    )
}
...
pub(crate) fn bus_subcall(
    ...
    super::bus_subcall::<MemoryType>(
        env, parent, topic_hash, format, buf, buf_len, ret_cid,
    )
}
...
pub(crate) fn bus_poll(
    ...
    events: WasmPtr<__wasi_busevent_t, MemoryType>,
...
pub(crate) fn call_fault(env: &WasiEnv, cid: __wasi_cid_t, fault: __bus_errno_t) {
    super::call_fault(env, cid, fault)
}

pub(crate) fn call_close(env: &WasiEnv, cid: __wasi_cid_t) {
