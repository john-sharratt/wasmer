...
impl Run {
    ...
    fn from_binfmt_args_fallible() -> Result<Run> {
        ...
        let args = args.iter().enumerate().map(|(i, s)| {
            s.clone().into_string().map_err(|s| anyhow!("Cannot convert argument {} ({:?}) to UTF-8 string", i + 1, s))
        }).collect::<Result<Vec<_>>>()?;
        let original_executable = original_executable.clone().into_string()
